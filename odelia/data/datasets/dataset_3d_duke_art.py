...
class DUKE_Dataset3D_ART(SimpleDataset3D):
    def __init__(self, path_root, item_pointers=[], crawler_glob='*.nii.gz', transform=None, image_resize=None, flip=False, image_crop=None, norm='znorm_clip', to_tensor=True):
        ...
        df = df[[df.columns[0], df.columns[57]]] # Only pick relevant columns: Patient ID, Tumor Side, Bilateral
        df.columns = ['PatientID', 'Adjuvant']  # Simplify columns as: Patient ID, Tumor Side
        dfs = []
        ...
        for side in ["left", 'right']:
            dfs.append(pd.DataFrame({
                ...
                # apply true or false to the 'Adjuvant' column
                'Adjuvant':df["Adjuvant"].apply(lambda ds: ds == 1)
            } ))
    ...
    def __getitem__(self, index):
        ...
        target = self.df.loc[uid]['Adjuvant']
    ...
    def get_labels(self):
        ...
        return self.df['Adjuvant'].values
    def __len__(self):
