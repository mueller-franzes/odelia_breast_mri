...
# Setting 
path_root = Path(
    '/mnt/sda1/swarm-learning/radiology-dataset/original-dataset/manifest-1654812109500/Duke-Breast-Cancer-MRI/')
...
metadatakeys = [
    ...
    'ImagingFrequency', 'InPlanePhaseEncodingDirection', 'InstitutionName', 'MRAcquisitionType',
    'MagneticFieldStrength',
    'Manufacturer', 'ManufacturerModelName', 'Modality', 'NumberOfAverages', 'NumberOfPhaseEncodingSteps',
    'NumberOfSlices',
    'PatientAge', 'PatientBirthDate', 'PatientID', 'PatientName', 'PatientSex', 'PatientWeight',
...
df_path2name = pd.read_excel(Path(
...
for case_i, path_dir in enumerate(tqdm(list(path_root_in.iterdir()))):
    ...
    dyn0_nii = sitk.ReadImage(str(path_out_dir / 'pre.nii.gz'),
                              sitk.sitkInt16)  # Note: if dtype not specified, data is read as uint16 -> subtraction wrong
    ...
    t1_resampled_nii = sitk.Resample(t1_nii, dyn0_nii, sitk.Transform(), sitk.sitkBSpline, 0,
                                     dyn0_nii.GetPixelID())  # Interpolation: sitk.sitkBSpline, sitk.sitkLinear
