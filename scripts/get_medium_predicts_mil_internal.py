...
parent_folder = '/mnt/sda1/Duke Compare/mil/trained_models/'
for node in ['Host_100/', 'Host_101/', 'merged/','Host_Sentinal/']:
    main_folder = parent_folder + node
    model_list = ['_transformer', '_timmViT', '_attmil']
    for model in model_list:
        ...
        for train_type in train_type_list:
            ...
            folders = glob.glob(main_folder+'*'+model+train_type)
            print(folders)
            # get the results.csv
            csv_files = []
            ...
            df_first.reset_index().rename(columns={'PATIENT': 'uid'}).to_csv(
                parent_folder + node.strip("/") + model.strip("/") + train_type + '_result.csv', index=False)

            # write the DataFrame to a new CSV file
            df_first.to_csv(parent_folder+node.strip("/")+model.strip("/")+train_type+'_result.csv', index=False)
