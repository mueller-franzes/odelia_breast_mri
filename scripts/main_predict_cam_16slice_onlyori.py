...
if __name__ == "__main__":
    ...
    if args.path_out:
        ...
        print(path_out)
    path_out = Path(
        '/mnt/sda1/Duke Compare/ext_val_occlusion_sensitivity/fp')
    ...
    ds = DUKE_Dataset3D_external(
        flip=False,
        path_root='/mnt/sda1/Oliver/data_fp'
    )
        ...
        for batch in tqdm(dm.test_dataloader()):
            ...
            for row, (im, title) in enumerate(
                ...
                for slice_index in range(start_slice, end_slice):
                    ...
                    ax.axis("off")
            # save cam_result
            plt.savefig(f"{path_out}/{uid}_{target_layers}_visall.svg", bbox_inches="tight", pad_inches=0)
