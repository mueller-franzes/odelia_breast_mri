...
if __name__ == "__main__":
    ...
    if args.path_out:
        ...
        print(path_out)
    path_out = Path(
        '/mnt/sda1/Duke Compare/ext_val_occlusion_sensitivity/inversecolor')
    ...
    ds = DUKE_Dataset3D_external(
        flip=False,
        path_root='/mnt/sda1/Oliver/data_top_worst'
    )
    ...
    for name in target_layers_list:
        # print(name, param.shape)
        target_layers = name
        gradcam = monai.visualize.GradCAM(nn_module=model, target_layers='model.layer3.0')
        # cam = monai.visualize.CAM(nn_module=model, target_layers=target_layers, fc_layers='model.fc')
        gradcampp = monai.visualize.GradCAMpp(nn_module=model, target_layers='model.layer3.1')
        ...
        for batch in tqdm(dm.test_dataloader()):
            ...
            for row, (im, title) in enumerate(
                    zip(
                        [img, gradcam_result_tensor, gradcampp_result_tensor],
                        [name, "GradCam", "GradCam++"],
                    )
            ):
